...
RocketChat.Assets = new ((function() {                                                                            // 1
  ...
  _Class.prototype.setAsset = function(binaryContent, contentType, asset) {                                       // 79
    ...
    if (assets[asset] == null) {                                                                                  // 83
      throw new Meteor.Error("error-invalid-asset", 'Invalid asset', {                                            // 84
        "function": 'RocketChat.Assets.setAsset'                                                                  // 84
      });                                                                                                         //
    }                                                                                                             //
    ...
    if (indexOf.call(assets[asset].constraints.extensions, extension) < 0) {                                      // 87
      throw new Meteor.Error(contentType, 'Invalid file type: ' + contentType, {                                  // 88
        "function": 'RocketChat.Assets.setAsset',                                                                 // 88
        errorTitle: 'error-invalid-file-type'                                                                     // 88
      });                                                                                                         //
    }                                                                                                             //
    ...
    if ((assets[asset].constraints.width != null) || (assets[asset].constraints.height != null)) {                // 91
      ...
      if ((assets[asset].constraints.width != null) && assets[asset].constraints.width !== dimensions.width) {    // 94
        throw new Meteor.Error("error-invalid-file-width", "Invalid file width", {                                // 95
          "function": 'Invalid file width'                                                                        // 95
        });                                                                                                       //
      }                                                                                                           //
  ...
  _Class.prototype.unsetAsset = function(asset) {                                                                 // 79
    if (assets[asset] == null) {                                                                                  // 115
      throw new Meteor.Error("error-invalid-asset", 'Invalid asset', {                                            // 116
        "function": 'RocketChat.Assets.unsetAsset'                                                                // 116
      });                                                                                                         //
    }                                                                                                             //
...
Meteor.methods({                                                                                                  // 1
  refreshClients: function() {                                                                                    // 219
    ...
    if (!Meteor.userId()) {                                                                                       // 220
      throw new Meteor.Error('error-invalid-user', "Invalid user", {                                              // 221
        method: 'refreshClients'                                                                                  // 221
      });                                                                                                         //
    }                                                                                                             //
    hasPermission = RocketChat.authz.hasPermission(Meteor.userId(), 'manage-assets');                             // 220
    if (!hasPermission) {                                                                                         // 224
      throw new Meteor.Error('error-action-now-allowed', 'Managing assets not allowed', {                         // 225
        method: 'refreshClients',                                                                                 // 225
        action: 'Managing_assets'                                                                                 // 225
      });                                                                                                         //
    }                                                                                                             //
  ...
  unsetAsset: function(asset) {                                                                                   // 219
    ...
    if (!Meteor.userId()) {                                                                                       // 231
      throw new Meteor.Error('error-invalid-user', "Invalid user", {                                              // 232
        method: 'unsetAsset'                                                                                      // 232
      });                                                                                                         //
    }                                                                                                             //
    hasPermission = RocketChat.authz.hasPermission(Meteor.userId(), 'manage-assets');                             // 231
    if (!hasPermission) {                                                                                         // 235
      throw new Meteor.Error('error-action-now-allowed', 'Managing assets not allowed', {                         // 236
        method: 'unsetAsset',                                                                                     // 236
        action: 'Managing_assets'                                                                                 // 236
      });                                                                                                         //
    }                                                                                                             //
  ...
  setAsset: function(binaryContent, contentType, asset) {                                                         // 219
    ...
    if (!Meteor.userId()) {                                                                                       // 242
      throw new Meteor.Error('error-invalid-user', "Invalid user", {                                              // 243
        method: 'setAsset'                                                                                        // 243
      });                                                                                                         //
    }                                                                                                             //
    hasPermission = RocketChat.authz.hasPermission(Meteor.userId(), 'manage-assets');                             // 242
    if (!hasPermission) {                                                                                         // 246
      throw new Meteor.Error('error-action-now-allowed', 'Managing assets not allowed', {                         // 247
        method: 'setAsset',                                                                                       // 247
        action: 'Managing_assets'                                                                                 // 247
      });                                                                                                         //
    }                                                                                                             //
