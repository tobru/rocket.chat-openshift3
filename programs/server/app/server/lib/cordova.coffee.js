...
sendPush = function(service, token, options, tries) {                  // 1
  ...
  return HTTP.post(RocketChat.settings.get('Push_gateway') + ("/push/" + service + "/send"), data, function(error, response) {
    var milli;                                                         // 139
    if ((response != null ? response.statusCode : void 0) === 406) {   // 139
      console.log('removing push token', token);                       // 140
      Push.appCollection.remove({                                      // 140
        $or: [                                                         // 141
          {                                                            //
            'token.apn': token                                         // 143
          }, {                                                         //
